...
int main(int argc, char *argv[])
{
    ...
    char *nonce_file = "solution_nonce.txt";
    char *iter_file = "solution_iterations.txt";
    int length;

    // Read challenge and convert to bytes
    char *challenge_hex = Read_File(challenge_file, &length);
    unsigned char *challenge = malloc(length / 2);
    int challenge_len = Hex_to_Bytes(challenge_hex, challenge, length);

    Print_Hex("Challenge", challenge, length / 2);

    // Read difficulty
    int k = Read_Int_From_File(difficulty_file);
    printf("Difficulty (k): %d\n", k);
    ...
    for (uint64_t nonce = 0; nonce < (1 << k); ++nonce)
    {
        memcpy(data, challenge, challenge_len);
        memcpy(data + challenge_len, &nonce, 8);
    ...
    char nonce_hex[17];
    Bytes_to_Hex((unsigned char *)&solved_nonce, 8, nonce_hex);
    Write_File(nonce_file, nonce_hex);
    Write_Int_To_File(iter_file, solved_nonce + 1);
